...
export async function DocumentExpiryList({ documents }: { documents: Doc[] }) {
  if (documents.length === 0) {
    return <p className="text-xs text-stone">No documents uploaded yet.</p>;
  }
  ...
  return (
    <ul className="space-y-2">
      {withUrls.map((d) => {
        ...
        return (
          <li key={d.id} className="flex items-start justify-between gap-2 text-xs border-b border-line pb-2 last:border-0 last:pb-0">
            <div className="min-w-0">
              {d.url ? (
                <a href={d.url} target="_blank" rel="noreferrer" className="text-ink font-medium hover:underline truncate block">
                  {d.document_subtype || d.file_name}
                </a>
              ) : (
                <span className="text-ink font-medium truncate block">{d.document_subtype || d.file_name}</span>
              )}
              {d.expiry_date ? <span className="text-stone">Expires {formatDate(d.expiry_date)}</span> : null}
            </div>
