...
export function PriceSuggestionPanel({
  ...
  return (
    <div className="border border-line rounded-sm p-3 bg-ivory-deep/40 space-y-3">
      <div className="flex items-center gap-1.5 text-xs font-semibold text-ink">
        <Sparkles className="h-3.5 w-3.5 text-gold" /> Pricing engine suggestion
      </div>
      <div className="flex flex-wrap items-end gap-2">
        <div>
          <label className="block text-[11px] text-stone mb-1">Vehicle category</label>
        ...
        <div>
          <label className="block text-[11px] text-stone mb-1">Distance (km, optional)</label>
          <input
            type="number"
            min={0}
            step="0.1"
        ...
        <button
          ...
          disabled={pending}
          className="text-xs bg-navy text-ivory px-3 py-1.5 rounded-sm hover:bg-navy-deep disabled:opacity-60"
        >
      ...
      {suggestion ? (
        <div className="text-xs space-y-1 border-t border-line pt-2">
          {suggestion.source === "NONE" ? (
            <p className="text-stone">No rate card or fixed route configured for {vehicleCategory} in {currency}. Set one up in Settings → Pricing engine, or price this manually.</p>
          ) : (
            <>
              <p className="text-stone">
                Matched: <span className="text-ink">{suggestion.matchedRuleLabel}</span> ({suggestion.source === "ROUTE_RATE" ? "fixed route" : "rate card"})
              </p>
              ...
              {suggestion.distanceCharge > 0 ? (
                <div className="flex justify-between">
                  <span className="text-stone">Distance ({distanceKm} km)</span>
