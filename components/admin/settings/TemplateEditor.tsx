    ...
    <div className="border border-line rounded-sm">
      <button type="button" onClick={() => setOpen((o) => !o)} className="w-full flex items-center justify-between px-4 py-3 text-left">
        <div>
          ...
          <p className="text-xs text-stone">{template.is_customized ? "Customized" : "Factory default"}</p>
        </div>
        <span className="text-xs text-gold">{open ? "Collapse" : "Edit"}</span>
      </button>

      {open ? (
        <div className="border-t border-line p-4 grid md:grid-cols-2 gap-4">
          <form action={updateNotificationTemplate.bind(null, template.key)} className="space-y-3">
            <div>
              <label className="block text-xs text-stone mb-1">Subject</label>
              <input name="subject_template" value={subject} onChange={(e) => setSubject(e.target.value)} className="input-luxe" />
            </div>
            <div>
              <label className="block text-xs text-stone mb-1">Body (blank line = new paragraph)</label>
            ...
            <div className="flex items-center gap-2">
              <button type="submit" className="text-sm bg-navy text-ivory px-4 py-2 rounded-sm hover:bg-navy-deep">
                Save
              </button>
          ...
          <div>
            <p className="text-xs text-stone mb-1">Preview (sample data)</p>
            <div className="border border-line rounded-sm p-3 bg-ivory-deep/40 text-sm space-y-2">
              <p className="font-semibold text-ink">{fillPreview(subject)}</p>
              <div className="whitespace-pre-wrap text-ink-soft">{fillPreview(body)}</div>
            </div>
          </div>
        </div>
      ) : null}
