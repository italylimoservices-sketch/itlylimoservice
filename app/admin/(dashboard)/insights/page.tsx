    ...
    <div>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mb-4">
            ...
            {funnel.map((stage, i) => {
              ...
              return (
                <div key={stage.label}>
                  ...
                  <div className="h-2.5 bg-ivory-deep rounded-full overflow-hidden">
                    <div className="h-full bg-gold rounded-full" style={{ width: `${(stage.count / maxFunnel) * 100}%` }} />
                  </div>
                </div>
              );
            })}
        ...
        <Section title="Profitability">
          ...
          <div className="px-4 pb-4 text-xs text-stone space-y-1">
              ...
              {topRoutes.map((r) => (
                <li key={r.route} className="px-4 py-2.5">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-stone shrink-0">{r.trips}</span>
                  </div>
                  <div className="h-1.5 bg-ivory-deep rounded-full overflow-hidden mt-1">
                    <div className="h-full bg-navy rounded-full" style={{ width: `${(r.trips / maxRouteTrips) * 100}%` }} />
                  </div>
                </li>
              ))}
            </ul>
            ...
            <ul className="divide-y divide-line">
              {topDrivers.map((d) => (
                <li key={d.id} className="px-4 py-2.5 flex justify-between text-sm">
                  ...
                  <span className="text-stone shrink-0 text-right">
                    {d.trips} trips
            ...
            <ul className="divide-y divide-line">
              {topVehicles.map((v) => (
                <li key={v.id} className="px-4 py-2.5 flex justify-between text-sm">
                  ...
                  <span className="text-stone shrink-0">{formatCurrency(v.revenue)}</span>
                </li>
              ))}
            </ul>
          )}
...
function ExportLink({ type, range }: { type: string; range: DateRangeKey }) {
  return (
    <Link href={`/admin/reports/export?type=${type}&range=${range}`} className="text-xs text-gold hover:underline">
      Export CSV
    </Link>
  );
}
