      ...
      <PageHeader
        ...
        actions={
          canManageCrm(profile.role) ? (
            <Link href="/admin/quotations/new" className="inline-flex items-center gap-1.5 bg-navy text-ivory text-sm font-semibold px-4 py-2 rounded-sm hover:bg-navy-deep">
          ...
          columns={[
            {
              header: "Number",
              cell: (q) => (
                <Link href={`/admin/quotations/${q.id}`} className="font-medium text-ink hover:text-gold">
                  {q.quotation_number}
                </Link>
              ),
            },
