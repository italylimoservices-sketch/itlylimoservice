      ...
      <PageHeader
        ...
        actions={
          <form className="flex items-center gap-2">
            ...
            <button type="submit" className="border border-line px-3 py-2 rounded-sm text-sm hover:bg-white">
              Go
            </button>
          </form>
        }
                  ...
                  return (
                    <tr key={t.id} className={unassigned ? "bg-amber-50/60" : ""}>
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/admin/bookings/${t.id}`} className="text-ink hover:text-gold font-medium">
                          {t.booking_reference}
                        </Link>
                      </td>
                      <td className="px-4 py-3">
                        <p>{t.customers?.full_name}</p>
                        <p className="text-xs text-stone">{t.customers?.phone}</p>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <form action={assignDriverAndVehicle.bind(null, t.id)} className="flex flex-col gap-1.5">
                          ...
                          <button type="submit" className="text-xs self-start text-gold hover:underline">
                            Save
                          </button>
                        </form>
                      </td>
