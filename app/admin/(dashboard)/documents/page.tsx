  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-4 text-sm">
        <Link href="/admin/documents" className={`px-3 py-1.5 rounded-sm border ${!entity_type ? "border-navy bg-navy text-ivory" : "border-line hover:bg-white"}`}>
          All
        </Link>
        {Object.keys(ENTITY_ROUTES).map((t) => (
          <Link key={t} href={`/admin/documents?entity_type=${t}`} className={`px-3 py-1.5 rounded-sm border capitalize ${entity_type === t ? "border-navy bg-navy text-ivory" : "border-line hover:bg-white"}`}>
            {t}
          </Link>
        ))}
      </div>
            ...
            { header: "Type", cell: (d: any) => d.doc_type.replaceAll("_", " ") },
            {
              ...
              cell: (d: any) =>
                ENTITY_ROUTES[d.entity_type] ? (
                  <Link href={`${ENTITY_ROUTES[d.entity_type]}/${d.entity_id}`} className="text-gold hover:underline capitalize">
                    {d.entity_type}
                  </Link>
                ) : (
                  d.entity_type
            ...
            { header: "Uploaded", cell: (d: any) => formatDateTime(d.created_at) },
            {
              ...
              cell: (d: any) => (
                <div className="flex items-center gap-3">
                  <a href={`/admin/documents/${d.id}/download`} target="_blank" className="text-stone hover:text-gold" aria-label="Download">
