              ...
              <ul className="divide-y divide-line">
                {pendingEarnings.map((e) => (
                  <li key={e.id} className="px-4 py-3 flex items-center justify-between gap-3 text-sm">
                    <div>
                      <p className="font-medium text-ink">{(e as any).drivers?.full_name ?? "Unknown driver"}</p>
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <form action={approveDriverEarning.bind(null, e.id)}>
                        <button type="submit" className="text-xs border border-line px-2.5 py-1 rounded-sm hover:bg-ivory-deep">
                          Approve
                        </button>
                      </form>
                    </div>
                ...
                {payouts.map((p) => (
                  <li key={p.id} className="px-4 py-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-medium text-ink">{(p as any).drivers?.full_name ?? "Unknown driver"}</p>
                        <p className="text-xs text-stone">
                        ...
                        {p.payment_reference ? <p className="text-xs text-stone">Ref: {p.payment_reference}</p> : null}
                      </div>
                      <div className="text-right shrink-0">
                        <StatusBadge status={p.status} />
                        <p className="text-sm font-semibold text-ink mt-1.5">{formatCurrency(p.net_payout, p.currency)}</p>
                      </div>
                    </div>
                    <div className="mt-2 flex gap-2">
                      ...
                      {p.status === "APPROVED" ? (
                        <form action={markDriverPayoutPaid.bind(null, p.id)} className="flex items-center gap-2">
                          ...
                          <ConfirmButton confirmMessage="Mark this payout as paid?" className="text-xs bg-navy text-ivory px-2.5 py-1 rounded-sm hover:bg-navy-deep">
                            Mark paid
                          </ConfirmButton>
                        </form>
                      ) : null}
          ...
          <Section title="New payout batch">
            <form action={createDriverPayout} className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs text-stone mb-1">Period start</label>
                  <input type="date" name="period_start" required className="input-luxe" />
                </div>
                <div>
                  <label className="block text-xs text-stone mb-1">Period end</label>
                  <input type="date" name="period_end" required className="input-luxe" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs text-stone mb-1">Adjustments (+/-)</label>
                  <input type="number" name="adjustments" step="0.01" defaultValue="0" className="input-luxe" />
                </div>
                <div>
                  <label className="block text-xs text-stone mb-1">Deduct expenses</label>
              ...
              <div>
                <label className="block text-xs text-stone mb-1">Notes</label>
                <textarea name="notes" rows={2} className="input-luxe" />
              </div>
              <p className="text-[11px] text-stone">Bundles every APPROVED, not-yet-paid earning for this driver whose trip date falls in the period.</p>
