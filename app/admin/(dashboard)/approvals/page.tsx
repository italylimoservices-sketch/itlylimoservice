            ...
            <ul className="divide-y divide-line">
              {expenses.map((e) => (
                <li key={e.id} className="px-4 py-3 flex justify-between text-sm">
                  <div>
                    <p className="text-ink">{e.category} · {formatDate(e.expense_date)}</p>
                    <p className="text-xs text-stone">{e.description ?? "—"}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-medium">{formatCurrency(e.amount, e.currency)}</p>
                    <Link href="/admin/expenses" className="text-xs text-gold hover:underline">
                      Review →
                    </Link>
                  </div>
                </li>
            ...
            <ul className="divide-y divide-line">
              {refunds.map((r) => (
                <li key={r.id} className="px-4 py-3 flex justify-between text-sm">
                  <div>
                    <p className="text-ink">{(r as any).customers?.full_name ?? "—"} · {r.refund_type}</p>
              ...
              {discounts.map((d) => (
                <li key={d.id} className="px-4 py-3 flex items-start justify-between text-sm gap-3">
                  <div>
                    ...
                    <p className="text-xs text-stone">{d.reason}</p>
            ...
            <ul className="divide-y divide-line">
              {payouts.map((p) => (
                <li key={p.id} className="px-4 py-3 flex justify-between text-sm">
                  <div>
                    <p className="text-ink">{(p as any).drivers?.full_name ?? "—"}</p>
                  ...
                  <div className="text-right">
                    ...
                    <Link href="/admin/driver-payouts" className="text-xs text-gold hover:underline">
                      Review →
                    </Link>
                  </div>
                </li>
