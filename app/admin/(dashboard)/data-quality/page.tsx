              ...
              <ul className="divide-y divide-line max-h-72 overflow-y-auto">
                {f.rows.map((r) => (
                  <li key={r.id} className="px-4 py-2 text-sm flex justify-between">
                    <Link href={ENTITY_LINKS[f.category]?.(r.id) ?? "#"} className="text-ink hover:underline truncate">
                      {r.label}
                    </Link>
                    {r.detail ? <span className="text-xs text-stone shrink-0 ml-2">{r.detail}</span> : null}
                  </li>
                ))}
              </ul>
            )}
