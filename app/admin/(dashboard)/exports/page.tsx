  ...
  return (
    <div>
      ...
      <Section title="Quick export">
        <div className="p-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {QUICK_EXPORT_KEYS.map((key) => (
            <a
              ...
              className="flex items-center justify-between gap-2 border border-line rounded-sm px-4 py-3 text-sm hover:bg-ivory-deep transition-colors"
            >
              {REPORT_LABELS[key]}
        ...
        <p className="px-4 pb-4 text-xs text-stone">
          Need a different dataset? The full report browser at{" "}
          <Link href="/admin/reports" className="text-gold hover:underline">
            /admin/reports
          </Link>{" "}
                ...
                <tbody className="divide-y divide-line">
                  {logs.map((log) => (
                    <tr key={log.id}>
                      ...
                      <td className="px-4 py-2.5 text-xs text-stone">{log.filters ? JSON.stringify(log.filters) : "—"}</td>
                      <td className="px-4 py-2.5 text-right">{log.row_count}</td>
                      <td className="px-4 py-2.5">
                        <StatusBadge status={log.status} />
                      </td>
                      <td className="px-4 py-2.5 text-xs text-stone">{formatDateTime(log.created_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
