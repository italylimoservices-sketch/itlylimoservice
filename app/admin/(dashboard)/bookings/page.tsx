      ...
      <PageHeader
        ...
        actions={
          canManageOps(profile.role) ? (
            <Link href="/admin/bookings/new" className="inline-flex items-center gap-1.5 bg-navy text-ivory text-sm font-semibold px-4 py-2 rounded-sm hover:bg-navy-deep">
          ...
          columns={[
            {
              header: "Reference",
              cell: (b) => (
                <Link href={`/admin/bookings/${b.id}`} className="font-medium text-ink hover:text-gold">
                  {b.booking_reference}
                </Link>
              ),
            },
