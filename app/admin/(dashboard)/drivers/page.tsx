      ...
      <PageHeader
        ...
        actions={
          canManageOps(profile.role) ? (
            <Link href="/admin/drivers/new" className="inline-flex items-center gap-1.5 bg-navy text-ivory text-sm font-semibold px-4 py-2 rounded-sm hover:bg-navy-deep">
          ...
          columns={[
            {
              header: "Name",
              cell: (d) => (
                <Link href={`/admin/drivers/${d.id}`} className="font-medium text-ink hover:text-gold">
                  {d.full_name}
                </Link>
              ),
            },
