                  ...
                  <li key={r.id} className="px-4 py-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        {r.description ? <p className="text-xs text-stone mt-1">{r.description}</p> : null}
                        {r.next_service_date ? (
                          <p className="text-xs text-amber-700 mt-1">Next service: {formatDate(r.next_service_date)}</p>
                        ) : null}
                      </div>
                      <span className="text-sm font-medium text-ink shrink-0">{formatCurrency(r.cost, r.currency)}</span>
                    </div>
                  </li>
                ))}
              </ul>
        ...
        <div>
          <Section title="Log service">
            <form action={createMaintenanceRecord} className="p-4 space-y-3">
              ...
              <div>
                <label className="block text-xs text-stone mb-1">Service date</label>
                <input type="date" name="service_date" required defaultValue={today} className="input-luxe" />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs text-stone mb-1">Mileage</label>
                  <input type="number" name="mileage" min={0} className="input-luxe" />
                </div>
                <div>
                  <label className="block text-xs text-stone mb-1">Cost</label>
                  <input type="number" name="cost" min={0} step="0.01" defaultValue="0" className="input-luxe" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-stone mb-1">Vendor</label>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs text-stone mb-1">Next service date</label>
                  <input type="date" name="next_service_date" className="input-luxe" />
                </div>
                <div>
                  <label className="block text-xs text-stone mb-1">Next service mileage</label>
