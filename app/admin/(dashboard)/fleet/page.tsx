      ...
      <PageHeader
        ...
        actions={
          <form className="flex items-center gap-2">
            ...
            <Link href="/admin/fleet/maintenance" className="text-sm bg-navy text-ivory px-4 py-2 rounded-sm hover:bg-navy-deep">
              Maintenance
            </Link>
          </form>
        }
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-line">
                {fleet.map((v) => (
                  <tr key={v.vehicleId} className="hover:bg-ivory-deep">
                    <td className="px-4 py-2.5">
                      <Link href={`/admin/vehicles/${v.vehicleId}`} className="text-ink hover:underline font-medium">
                        {v.name}
                      </Link>
                    </td>
                    <td className="px-4 py-2.5">
                    ...
                    <td className="px-4 py-2.5 text-right text-stone">{formatCurrency(v.fuelCost)}</td>
                    <td className="px-4 py-2.5 text-right text-stone">{formatCurrency(v.maintenanceCost)}</td>
                    <td className="px-4 py-2.5 text-right text-stone">{formatCurrency(v.tollsCost)}</td>
                    <td className="px-4 py-2.5 text-right text-stone">{formatCurrency(v.otherCost)}</td>
