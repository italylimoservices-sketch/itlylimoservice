        ...
        <div className="space-y-4">
          <Section title="Users &amp; roles">
            <div className="p-4 space-y-3">
              <p className="text-sm text-stone">Manage admin accounts, roles, and activation.</p>
              <Link href="/admin/users" className="inline-block text-sm bg-navy text-ivory px-3 py-2 rounded-sm hover:bg-navy-deep">
                Manage users
              </Link>
              ...
              <div className="pt-2 border-t border-line space-y-2">
                {PERMISSION_MATRIX.map(([role, desc]) => (
                  <div key={role} className="text-xs">
                    <p className="font-semibold text-ink">{ROLE_LABELS[role as keyof typeof ROLE_LABELS]}</p>
                    <p className="text-stone">{desc}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <Section title="Notifications &amp; email">
            <div className="p-4 text-sm text-stone space-y-2">
              <p>
