            ...
            <Card key={d.key} className="p-4">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-stone mt-0.5">{d.description}</p>
                ...
                <form action={setAutomationEnabled.bind(null, d.key)}>
                  ...
                  <button
                    ...
                    className={`text-xs px-2.5 py-1 rounded-full border shrink-0 ${
                      d.enabled ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-line text-stone"
                    }`}
                  >
              ...
              {DELAY_RULES[d.key] ? (
                <form
                  ...
                  className="mt-3 pt-3 border-t border-line flex items-center gap-2 text-xs text-stone"
                >
              ...
              <ul className="divide-y divide-line">
                {runs.map((r) => (
                  <li key={r.id} className="px-4 py-3 flex items-center justify-between gap-3 text-sm">
                    <div>
                      <p className="font-medium text-ink">{r.automation_key}</p>
