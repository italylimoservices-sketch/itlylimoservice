      ...
      <PageHeader
        ...
        actions={
          canManageOps(profile.role) ? (
            <Link href="/admin/vehicles/new" className="inline-flex items-center gap-1.5 bg-navy text-ivory text-sm font-semibold px-4 py-2 rounded-sm hover:bg-navy-deep">
          ...
          columns={[
            {
              header: "Name",
              cell: (v) => (
                <Link href={`/admin/vehicles/${v.id}`} className="font-medium text-ink hover:text-gold">
                  {v.name}
                </Link>
              ),
            },
