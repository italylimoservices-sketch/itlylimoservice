          ...
          <Section title="Vehicle details">
            <div className="p-4">
              {canEdit ? (
                ...
                <p className="text-sm text-stone">{vehicle.registration_number}</p>
              )}
            </div>
          </Section>
                ...
                {STATUSES.map((s) => (
                  <form key={s} action={setVehicleStatus.bind(null, id, s)}>
                    <button
                      ...
                      className={`text-xs px-2.5 py-1.5 rounded-sm border ${vehicle.status === s ? "border-navy bg-navy text-ivory" : "border-line hover:bg-ivory-deep"}`}
                    >
                      {s}
                    </button>
                  </form>
