                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-stone">{r.source.replaceAll("_", " ")}</span>
                        </div>
                        <p className="text-sm text-ink mt-1">{r.comment ?? <span className="text-stone italic">No comment</span>}</p>
                      ...
                      <form action={setReviewPublished.bind(null, r.id)}>
                        ...
                        <button
                          ...
                          className={`text-xs px-2.5 py-1 rounded-full border shrink-0 ${
                            r.is_published ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-line text-stone"
                          }`}
                        >
        ...
        <div>
          <Section title="Log a review">
            <form action={createStaffReview} className="p-4 space-y-3">
              <div>
                <label className="block text-xs text-stone mb-1">Customer</label>
                <EntityPicker entity="customers" name="customer_id" placeholder="Search customers…" required />
              </div>
              <div>
                <label className="block text-xs text-stone mb-1">Booking (optional)</label>
                <EntityPicker entity="bookings" name="booking_id" placeholder="Search bookings…" />
              </div>
              <div>
                <label className="block text-xs text-stone mb-1">Rating</label>
              ...
              <div>
                <label className="block text-xs text-stone mb-1">Comment</label>
                <textarea name="comment" rows={3} className="input-luxe" placeholder="What the customer actually said" />
              </div>
              <div>
                <label className="block text-xs text-stone mb-1">Source</label>
