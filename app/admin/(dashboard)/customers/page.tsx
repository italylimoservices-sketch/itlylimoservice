                ...
                {customers.map((c) => (
                  <tr key={c.id} className="hover:bg-ivory-deep">
                    <td className="px-4 py-3">
                      <Link href={`/admin/customers/${c.id}`} className="font-medium text-ink hover:text-gold">
                        {c.full_name}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-stone">{c.company_name ?? "—"}</td>
                    <td className="px-4 py-3 text-stone">{c.email ?? "—"}</td>
                    <td className="px-4 py-3 text-stone">{c.phone ?? "—"}</td>
                    <td className="px-4 py-3 text-stone">{formatDate(c.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
