            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-line">
                  {displayRows.map((c) => (
                    <tr key={c.id} className="hover:bg-ivory-deep">
                      <td className="px-4 py-2.5">
                        <Link href={`/admin/customers/${c.id}`} className="text-ink hover:underline">
                          {c.full_name}
                        </Link>
                      </td>
            ...
            <ul className="divide-y divide-line">
              {overrides.map((o) => (
                <li key={o.customer_id} className="px-4 py-3 flex items-center justify-between text-sm gap-3">
                  <div>
                    <Link href={`/admin/customers/${o.customer_id}`} className="text-ink hover:underline font-medium">
                      {o.customers?.full_name ?? "—"}
                    </Link>
                    {o.note ? <p className="text-xs text-stone mt-0.5">{o.note}</p> : null}
                  </div>
