          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-line">
                {rows.map((r) => (
                  <tr key={r.id} className="hover:bg-ivory-deep">
                    <td className="px-4 py-2.5">
                      <Link href={`/admin/customers/${(r as any).customers?.id}`} className="text-ink hover:underline">
                        {(r as any).customers?.full_name ?? "—"}
                      </Link>
                    </td>
                    <td className="px-4 py-2.5">
                      <Link href={`/admin/invoices/${r.id}`} className="text-gold hover:underline">
                        {r.invoice_number}
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-right text-stone">{formatCurrency(r.amount_paid, r.currency)}</td>
