    ...
    <div>
      <PageHeader
        ...
        actions={
          <form className="flex items-center gap-2">
            ...
            <button type="submit" className="border border-line px-4 py-2 rounded-sm text-sm hover:bg-white">
              Apply
            </button>
          </form>
        }
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-4">
        <Section title="Quick links">
          <div className="p-4 flex flex-wrap gap-2 text-sm">
            <Link href="/admin/finance/receivables" className="border border-line px-3 py-1.5 rounded-sm hover:bg-ivory-deep">
              Accounts receivable
            </Link>
            <Link href="/admin/finance/reconciliation" className="border border-line px-3 py-1.5 rounded-sm hover:bg-ivory-deep">
              Reconciliation
            </Link>
            <Link href="/admin/driver-payouts" className="border border-line px-3 py-1.5 rounded-sm hover:bg-ivory-deep">
              Driver payouts
            </Link>
            <Link href="/admin/expenses" className="border border-line px-3 py-1.5 rounded-sm hover:bg-ivory-deep">
              Expenses
            </Link>
          </div>
        </Section>
        ...
        <Section title="Expense breakdown">
          <ul className="p-4 space-y-1.5 text-sm">
            ...
            {categoryTotals.size === 0 ? <li className="text-stone">No approved expenses in this period.</li> : null}
          </ul>
        </Section>
          ...
          <ul className="p-4 space-y-1.5 text-sm">
            {finance.revenueByCurrency.map((r) => (
              <li key={r.currency} className="flex justify-between">
                <span className="text-stone">{r.currency}</span>
                <span>{formatCurrency(r.amount, r.currency)}</span>
              </li>
            ))}
            {finance.revenueByCurrency.length === 0 ? <li className="text-stone">No revenue in this period.</li> : null}
          </ul>
        </Section>
      </div>
      ...
      <Section title="Recent payments">
        <ul className="divide-y divide-line">
          ...
          {!recentPayments || recentPayments.length === 0 ? <li className="px-4 py-4 text-sm text-stone">No payments in this period.</li> : null}
        </ul>
      </Section>
    </div>
  );
