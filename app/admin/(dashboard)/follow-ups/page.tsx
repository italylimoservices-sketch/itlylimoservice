    ...
    <div>
      ...
      <div className="flex gap-2 mb-4 text-sm">
        {["PENDING", "COMPLETED", "CANCELLED"].map((s) => (
          <Link key={s} href={`/admin/follow-ups?status=${s}`} className={`px-3 py-1.5 rounded-sm border ${activeStatus === s ? "border-navy bg-navy text-ivory" : "border-line hover:bg-white"}`}>
            {s}
          </Link>
        ))}
      </div>
                        ...
                        cell: (f: any) => (
                          <div className="flex gap-2">
                            ...
                            <form action={cancelFollowUp.bind(null, f.id)}>
                              <button type="submit" className="text-stone hover:text-red-600" aria-label="Cancel">
                                <X className="h-4 w-4" />
                              </button>
                            </form>
                          </div>
