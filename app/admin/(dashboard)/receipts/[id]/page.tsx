          ...
          <div className="flex items-center gap-2">
            {customer?.phone ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-sm border border-line px-3 py-2 rounded-sm hover:bg-white"
              >
                Send via WhatsApp
              </a>
            ) : null}
            <Link href={`/admin/receipts/${id}/pdf`} target="_blank" className="inline-flex items-center gap-1.5 text-sm border border-line px-3 py-2 rounded-sm hover:bg-white">
      ...
      <Card className="p-6 max-w-lg space-y-3 text-sm">
        ...
        {(receipt as any).invoices ? (
          <Row
            label="Invoice"
            value={
              <Link href={`/admin/invoices/${(receipt as any).invoices.id}`} className="text-gold hover:underline">
        ...
        {(receipt as any).bookings ? (
          <Row
            label="Booking"
            value={
              <Link href={`/admin/bookings/${(receipt as any).bookings.id}`} className="text-gold hover:underline">
